...
-- Feed orders data
WITH user_emails AS (SELECT id, email
                     FROM users
                     WHERE email IN (
                                     'test.user@mail.com',
...
INSERT
INTO orders (id, user_id, email, total, status, created_at, updated_at)
SELECT gen_random_uuid(),
       id,
       email,
       69.94,
...
WITH recent_orders AS (SELECT o.id as order_id, ROW_NUMBER() OVER () as rn
                       FROM orders o
                                JOIN users u ON o.user_id = u.id
                       WHERE u.email LIKE 'user%@mail.com'
                          OR u.email = 'test.user@mail.com'
                       ORDER BY o.created_at DESC
                       LIMIT 20)
INSERT
INTO order_items (id, product_id, order_id, name, quantity, price, total, is_reviewed)
SELECT gen_random_uuid(),
...
-- Feed reviews data
WITH user_order_data AS (SELECT DISTINCT u.id as user_id,
                                         u.email,
                                         o.id as order_id
                         FROM users u
                         ...
                         LIMIT 20)
INSERT
INTO reviews (id, user_id, product_id, order_id, email, rating, comment, created_at)
SELECT gen_random_uuid(),
       user_id,
       'b9c6ebbe-a540-415c-b698-fb7b88bd202d'::uuid,
       order_id,
